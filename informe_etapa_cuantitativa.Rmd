...
tabla$nse<-as.factor(tabla$nse)
levels(tabla$nse)<-c("Alto", "Medio alto", "Medio", "Medio Bajo", "Bajo")
...
datos_jp$composicion_menores_sexo_aportante<-
  ifelse(datos_jp$menores=="con menores" & datos_jp$sexo_aportante=="Mujer", 1,
         ifelse(datos_jp$menores=="sin menores" & datos_jp$sexo_aportante=="Mujer", 2,3))

datos_jp$composicion_menores_sexo_aportante<-as.factor(datos_jp$composicion_menores_sexo_aportante)
levels(datos_jp$composicion_menores_sexo_aportante)<-c("aportante mujer con menores en el hogar",
                                                       "aportante mujer sin menores en el hogar",
                                                       "otras combinaciones")


kable_styling(column_spec(kable(addmargins(table(
    datos_jp$composicion_menores_sexo_aportante, datos_jp$nse),1), caption = "Hogares cuya principal aportante es mujer con o sin menores en el hogar según NSE (absolutos)"), 1, width="5cm"), latex_options = "HOLD_position")


kable_styling(column_spec(kable(round(100*addmargins(prop.table(
  table(
    datos_jp$composicion_menores_sexo_aportante, datos_jp$nse),2),1),2), caption = "Hogares cuya principal aportante es mujer con o sin menores en el hogar (porcentajes según NSE)"), 1, width="5cm"), latex_options = "HOLD_position")


```

Nota: la categoría "otras combinaciones" incluye los casos en que no es posible identificar el sexo del o la principal aportante  
...
La combinación de acceso mediante datos y wifi compartido tiene presencia apreciable sólo en el sector medio bajo, con el 4,1% de los hogares. [Acá tendremos bastante para decir respecto de las calidades y costos de estos servicios y su incidencia en la desigualdad.]  
De esta distribucion se deduce, que, en la gran mayoría de los hogares pertenecientes a los sectores bajos y medio bajos, la continuidad educativa se garantizó mediante un dispositivo celular. Sin embargo, al considerar la cantidad de celulares por persona en el hogar cabe suponer que en algunos casos el uso del celular durante la pandemia ha sido un recurso compartido por más de un integrante del hogar.  
...
## Nuevas Tablas (ver dónde ubicar)


```{r, echo=F, include=F, message=F}
load("datos_nuevas_tablas.Rda")
datos_nuevas_tablas$cant_menores_14 = as.numeric(datos_nuevas_tablas$cant_menores_14)
datos_nuevas_tablas$ingresos_del_hogar = as.factor(datos_nuevas_tablas$ingresos_del_hogar)
datos_nuevas_tablas$ingresos_del_hogar = factor(datos_nuevas_tablas$ingresos_del_hogar, levels = c("Menos de $25.000","Entre $25000 y 55.000","Entre $55.000 y $99.000","Entre $100.000 y $200.000","Más de $200.000"))

### Tabla ingresos del hogar y cantidad promedio de personas según rango etario
tabla1 = datos_nuevas_tablas %>%
  filter(is.na(ingresos_del_hogar)==F) %>% 
  group_by(ingresos_del_hogar) %>%
  summarise("menos de 14 años" = mean(cant_menores_14, na.rm = T),
            "de 15 a 60 años" = mean(cant_14_60, na.rm = T),
            "más de 60 años" = mean(cant_mas_60, na.rm = T))
tabla1

### Tabla ingresos del hogar y cantidad promedio de perceptores de ingresos, por género
tabla2 = datos_nuevas_tablas %>%
  filter(is.na(ingresos_del_hogar)==F) %>% 
  group_by(ingresos_del_hogar) %>%
...
```
```{r}
kable_styling(kable(tabla1, caption = "Ingresos del hogar y cantidad promedio de personas según rango etario"), latex_options = c("HOLD_position","scale_down"))

kable_styling(kable(tabla2, caption = "Ingresos del hogar y cantidad promedio de perceptores de ingresos, por género"), latex_options = c("HOLD_position","scale_down"))

kable_styling(kable(tabla3, caption = "Ingresos del hogar y cantidad promedio de miembros de la vivienda (no hogar)"), latex_options = c("HOLD_position","scale_down"))
```

Tipo de Hogares y  hacinamiento / Hogares monoparentales con jefatura femenina (mujeres con niñxs a cargo) y  hacinamiento solo un integrante de 14 o más + con menores de 14 + jefatura femenina (de los identificados como tales)

```{r, include=T, echo=T}
datos$aux = ifelse(is.na(datos$`Cuántas tienen más de 60 años?`)==T, 0, datos$`Cuántas tienen más de 60 años?`)
datos$aux1 = ifelse(is.na(datos$`Cuántas tienen entre 14 y 60 años?`)==T, 0, datos$`Cuántas tienen entre 14 y 60 años?`)
datos$`Cuántas son menores de 14 años?` = as.numeric(datos$`Cuántas son menores de 14 años?`)
datos$aux2 = ifelse(is.na(datos$`Cuántas son menores de 14 años?`)==T, 0, datos$`Cuántas son menores de 14 años?`)
datos$aux3 = datos$aux + datos$aux1

datos$hogar_monomarental = ifelse(datos$aux2>0 & datos$aux3 > 0 & datos$`Varones que aportan ingresos`== 0 & datos$`Mujeres que aportan ingresos`== 1, 1, 0)

table(datos$hogar_monomarental)
